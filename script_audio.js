function comparison(speaker, emotion, emotion_eng, dataset) {
    ...
    raw_data.innerHTML = `<audio id="audio-player" controls="" preload="none" src="./wavs_${dataset.slice(0, -1)}_kor/gss/${speaker}/${emotion}.wav" type="audio/mpeg">`
    table1.appendChild(raw_data)
    
    const table2 = document.querySelector(`#${dataset} #b`)
    let raw_data2 = document.createElement('td')
    raw_data2.innerHTML = `<audio id="audio-player" controls="" preload="none" src="./wavs_${dataset.slice(0, -1)}_kor/cg/${speaker}/${emotion}.wav" type="audio/mpeg">`
...
function allEmotion(speaker, dataset, kor) {
    ...
    const table1 = document.querySelector(`#${dataset} #a`)
    let raw_data = document.createElement('td')
    raw_data.innerHTML = `<audio id="audio-player" controls="" preload="none" src="./wavs_${dataset}/cfg/${speaker}/무감정.wav" type="audio/mpeg">`
    table1.appendChild(raw_data)
    
    const table2 = document.querySelector(`#${dataset} #b`)
    let raw_data2 = document.createElement('td')
    raw_data2.innerHTML = `<audio id="audio-player" controls="" preload="none" src="./wavs_${dataset}/cfg/${speaker}/기쁨.wav" type="audio/mpeg">`
    table2.appendChild(raw_data2)
    
    const table3 = document.querySelector(`#${dataset} #c`)
    let raw_data3 = document.createElement('td')
    raw_data3.innerHTML = `<audio id="audio-player" controls="" preload="none" src="./wavs_${dataset}/cfg/${speaker}/슬픔.wav" type="audio/mpeg">`    
    table3.appendChild(raw_data3)

    const table4 = document.querySelector(`#${dataset} #d`)
    let raw_data4 = document.createElement('td')
    raw_data4.innerHTML = `<audio id="audio-player" controls="" preload="none" src="./wavs_${dataset}/cfg/${speaker}/분노.wav" type="audio/mpeg">`    
    ...
    if (kor) {
        ...
        let raw_data7 = document.createElement('td')
        raw_data7.innerHTML = `<audio id="audio-player" controls="" preload="none" src="./wavs_${dataset}/cfg/${speaker}/당황.wav" type="audio/mpeg">`    
        table7.appendChild(raw_data7)
    }
